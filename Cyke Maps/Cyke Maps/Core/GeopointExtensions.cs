...
namespace CykeMaps.Core
{
    public static class GeopointExtensions
    {
        ...
        public static Geopath ToGeoPath(
          this IList<Geopoint> pointList)
        {
            return new Geopath(pointList.Select(p => p.Position));
        }

        public static Point ToPoint(this Geopoint geopoint)
        {
            return new Point(geopoint.Position.Latitude, geopoint.Position.Longitude);
        }

        public static Geopoint ToGeopoint(this Point point)
        {
            return new Geopoint(new BasicGeoposition() { Latitude = point.X, Longitude = point.Y });
        }
    }
}
